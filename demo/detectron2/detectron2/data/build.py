...
def build_detection_test_loader(cfg, dataset_name, mapper=None):
    ...
    """
    dataset_dicts = get_detection_dataset_dicts(   # 获得dataset_dicts
        [dataset_name],    #dataset_names,这个参数其实就是一个字符串，用来指定数据集的名称
        filter_empty=False,
        proposal_files=[
    ...
    dataset = DatasetFromList(dataset_dicts)   ########将dataset_dicts转化成torch.utils.data.Dataset
    if mapper is None:
        mapper = DatasetMapper(cfg, False)
    dataset = MapDataset(dataset, mapper) # 进一步转化成MapDataset，每次读取数据时都会调用mapper来对dict进行解析

    sampler = samplers.InferenceSampler(len(dataset))  # 采样器
    # Always use 1 image per worker during inference since this is the
    # standard when reporting inference time in papers.
    batch_sampler = torch.utils.data.sampler.BatchSampler(sampler, 1, drop_last=False)

    data_loader = torch.utils.data.DataLoader(    # 数据迭代器 data_loader
        dataset,
        num_workers=cfg.DATALOADER.NUM_WORKERS,
        batch_sampler=batch_sampler,
        collate_fn=trivial_batch_collator,
